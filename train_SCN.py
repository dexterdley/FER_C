...
class Res18Feature(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.fc1(x)
        x2 = attention_weights * self.fc2(x) #usage

        #pdb.set_trace()
        return attention_weights, x1, x2 #attn weights, regression output, classification output
...
def run_training():
    ...
    train_dataset = FER_dataset(root, train_df, dataset=args.dataset, transform=val_transformations, SCN=True)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, GT_classes, GMM_labels, indexes) in tqdm(enumerate(train_loader)):
            ...
            # forward
            attention_weights, pred, logits = model(data)
            ...
            if epoch >= args.relabel_epoch:
                Pmax, predicted_labels = torch.max(class_pred, 1) # predictions
                Pgt = torch.gather(class_pred, 1, GT_classes.view(-1,1)).squeeze() # retrieve predicted probabilities of targets
                true_or_false = Pmax - Pgt > margin_2
                update_idx = true_or_false.nonzero().squeeze().view(-1) # get samples' index in this mini-batch where (Pmax - Pgt > margin_2)
                label_idx = indexes[update_idx.cpu()] # get samples' index in train_loader
                relabels = predicted_labels[update_idx.cpu()] # predictions where (Pmax - Pgt > margin_2)
                train_dataset.df['expression'][label_idx.cpu().numpy()] = relabels.cpu().numpy() # relabel samples in train_loader
                
                num_relabelled += len(label_idx)
            ### End of relabelling samples ###

        if num_relabelled > 0:
            print("Relabelled:", num_relabelled)
