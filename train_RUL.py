...
from losses import *
from rul_models import res18feature, mixup_criterion
...
def parse_args():
    ...
    parser.add_argument('--pretrained_backbone_path', type=str, default='./out_dir_res18/resnet18_msceleb.pth', help='pretrained_backbone_path')   
    ...
    parser.add_argument('--lr', type=float, default=2e-4, help='Initial learning rate for sgd.')
...
def run_training():
    ...
    model = res18feature(args, num_classes=num_classes)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, GT_classes, GMM_labels) in tqdm(enumerate(train_loader)):
            ...
            mixed_x, y_a, y_b, att1, att2, logits = model(data, GT_classes, phase='train')
            class_pred = F.softmax(logits, dim=1)

            #criterion = nn.CrossEntropyLoss()
            targets_a = convert_to_onehot(y_a, num_classes).to(device)
            targets_b = convert_to_onehot(y_b, num_classes).to(device)

            loss, CE_loss = mixup_criterion(criterion, class_pred, targets_a, targets_b, class_weights)
        ...
        ce_val_loss, scores, val_acc, val_F1, ECE, MCE, adaece, KSE = evaluate_model(val_loader, model, num_classes, algo='RUL')
    ...
    ce_test_loss, scores, test_acc, test_F1, test_ECE, test_MCE, test_adaece, test_KSE = evaluate_model(test_loader, best_model, num_classes, algo='RUL')
    # test on test set
    test_stats = ",\t".join([
